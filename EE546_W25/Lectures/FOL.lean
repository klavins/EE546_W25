...
where we create new propositions for every person and every statement we would like to say about that person. However, what if we wanted to reason about an infinite domain like Nat and say things like the following?

  every integer is either odd or even

Since there are an infinite number of Nats, we need an infinite number of  propositions odd_0, even_0, odd_1, even_1, ...
...
First order logic enriches propositional logic with the following elements:
  ...
  - `Quantifiers` ∀ and ∃ that allow us to say:
      - ∀: For all objects such that ___
      - ∃: There exists an object such that ___
  - All the connectives we have encountered so far: ∨, ∧, →, ¬, ...
  - (Traditional FOL does not have types, but we will use them anyway)

For example, in the following proposition built from these elements:

  ∀ x ∃ y , f x > y

is read "For all x, there exists a y such that f(x) is greater than y".
...
All of this can be done easily in L∃∀N. -/
...
/- # EXAMPLE : NATURAL NUMBERS, STRINGS, ETC -/
...
`EXAMPLE` A predicate on Person is a function from Person into Prop, such as one which might specify whether the person lives in Seattle: -/
